...
class Enemies:
    ...
    def create_enemies(self):
        for line in range(3):  # numéro de la ligne
            for column in range(10):  # numéro de la colonne
                ...
                self.shot_times.append(
                    random.randint(1000, 5000))  # permet à l'ennemi d'attaquer à partir d'un certain temps
    ...
    def shot(self):
        for index, enemy in enumerate(self.enemies):  # parcours le "tableau" d'ennemis
            ...
            if time_now - self.shot_clock[index] > self.shot_times[index]:  # quand le chrono est atteint
                ...
                self.shot_times[index] = random.randint(1000,
                                                        5000)  # attend aléatoirement entre 1 à 5 secondes avant de tirer
                if random.randint(0, 100) < 30:  # donne 30% de chances de tirer
                    if random.randint(0,
                                      100) < self.meteor_probability:  # vérifie si le pourcentage de chance de tirer un météore est atteint
    ...
    # même système que pour les projectiles du joueur
    ...
    def update(self):
        ...
        for i in range(len(self.enemies)):  # applique le déplacement à tous les éléments
            self.enemies[i][1] += self.curr_dir

        # si la vague a été entièrement tuée, en crée une nouvelle
    ...
    def bullet_collide(self, x, y):
        for index, enemy in enumerate(self.enemies):
            # détruit ce qui est relié à un ennemi, soit lui-même et les chronos associés
    ...
    def player_collision(self, x, y):
        ...
        for i, e in enumerate(self.enemies):
            if e[1] <= x + 50 <= e[1] + 50 and e[2] <= y + 50 <= e[2] + 50:
                del self.enemies[i]
                return -1
        ...
        for i, b in enumerate(self.bullets):
            ...
            # étoile
                ...
                if b[1] <= x + 50 <= b[1] + 30 and b[2] <= y + 50 <= b[2] + 30:
                    del self.bullets[i]
                    return 1
        return 0
